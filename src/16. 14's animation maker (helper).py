...
plots_dir = "plots/make_animation_3bavg_new"
os.makedirs(plots_dir, exist_ok=True)

lowerbound1 = 2097
numberofmaps = 187  # stops it at 2229
...
with tqdm(total=numberofmaps, desc="Processing Carrington Rotations") as pbar:
    for tempupperbound in range(0, numberofmaps):
        ...
        # Append image to frames list for GIF
        frames.append(Image.open(plot_path))

        pbar.update(1)

# Save all frames as a GIF
gif_path = os.path.join(plots_dir, "magnetic_flux_animation.gif")
frames[0].save(gif_path, save_all=True, append_images=frames[1:], duration=200, loop=0)

# Clean up temporary images
for tempupperbound in range(0, numberofmaps):
    temp_plot_path = os.path.join(plots_dir, f"temp_plot_{tempupperbound}.png")
