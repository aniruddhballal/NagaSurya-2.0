...
def calcalm(b_func, lmax, num_points_theta, num_points_phi, carrington_map_number, check, verbose=True):
    ...
    if check == 1:
        values_filename = os.path.join(values_folder, f'gauss_values_{carrington_map_number}.csv')
    else:
        values_filename = os.path.join(values_folder, f'gauss_predicted_values_{carrington_map_number}.csv')
...
def recons(alm, x, y, lmax, map_number, check, verbose=True):
    ...
    metadata_file = ""
    if check == 1:
        save_file = os.path.join(reconsdata_folder, f"gauss_brecons_og_{map_number}_{lmax}.npy")  # File to save reconstruction progress
        metadata_file = os.path.join(reconsdata_folder, f"gauss_brecons_og_{map_number}_{lmax}_metadata.json")  # Metadata file for tracking progress
    else:
        save_file = os.path.join(reconsdata_folder, f"gauss_brecons_pred_{map_number}_{lmax}.npy")  # File to save reconstruction progress
        metadata_file = os.path.join(reconsdata_folder, f"gauss_brecons_pred_{map_number}_{lmax}_metadata.json")  # Metadata file for tracking progress
